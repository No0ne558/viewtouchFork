...
### Added
- **Qt6 Terminal Renderer: Full Migration from Xlib/Xt to Qt6** (2026-06-18)
  - Replaced the legacy X11/Xt/Xlib terminal client (`vt_term`) with a fully Qt6-based renderer. The `vt_main` business logic, zone model, and CharQueue binary protocol are completely unchanged — only the display client is replaced.
  - **New files:** `term/qt_term_main.cc`, `term/qt_term_view.cc`, `term/qt_term_view.hh` — implement `TermWidget : QWidget` with QPainter-based rendering of the full `TERM_*` draw protocol (frames, textures, text, shadows, lines, image zones), plus touch/mouse/keyboard input forwarding and multi-window overlay support.
  - **Loader (`loader/loader_main.cc`):** Rewritten as a Qt6 `QDialog` splash screen with `QSocketNotifier` for non-blocking IPC. Removed Xlib/Motif dependency from the loader entirely.
  - **Dependency changes (`CMakeLists.txt`, `cmake/`, `ci/`):** Added Qt6 (Widgets, Gui, Core, Svg), SQLite3, and cpp-httplib; removed Xlib/Xt/Xft from the terminal build. Motif is now optional (only needed when vt_main's server code still calls Xft directly).
  - **curlpp removed (`main/data/manager.cc`):** Replaced with direct `libcurl` C API (`curl_easy_*`). Removes the `external/curlpp/` CMake sub-project.
  - **SQLite persistence** added for Drawers, Archives, Inventory, Employees, WorkEntries, Sales, Exceptions, and Settings — load falls back to `.dat` files and auto-migrates on first save. Single shared WAL-mode database at `<dat_dir>/viewtouch.db`.
  - **Web admin dashboard (`main/web/web_server.cc/hh`):** Embedded HTTP/1.1 server (background thread, select event loop). Endpoints: `GET /` dark-mode SPA (Overview/Menu/Reports tabs), `GET /cdu` customer-facing display, `/api/health`, `/api/sales`, `/api/checks`, `/api/employees`, `/api/menu`, `POST /api/menu/price`, `/api/reports/today`.
  - **CUPS/ESC-POS printer backend (`main/hardware/printer.cc/hh`):** Added `TARGET_CUPS` destination type (`cups:<queue>`), `Printer::CupsPrint()` calling `lp -d`, async dispatch through the thread pool. ESC/POS thermal printers use `-o raw` to bypass CUPS raster pipeline.
  - **Qt6 edit-mode dialogs** (`term/qt_term_view.cc`): Zone dialog (all 70+ zone types with Item tab for item zones), Page dialog, Defaults dialog, Multi-zone dialog, and List-selection dialog — all ported from Motif to Qt6 `QDialog`.
  - **Visual design refresh (Phase 4):** Noto Sans replaces DejaVu Serif; XPM tiling replaced by subtle vertical-gradient textures; 1–2 px flat frame borders; multi-layer soft drop-shadows; improved contrast colour palette.
  - **Initial Qt renderer bug fixes:** XPM texture tiling restored; backslash line-separators in zone labels fixed; `IMAGE_UNCHANGED`/`IMAGE_DEFAULT` black-zone bug fixed; `cmdBlankPage()` clip-state reset fixed; frame bevel thickness fixed; `QApplication::beep()` wired for `TERM_BELL`.
  - Files modified/added: `term/qt_term_main.cc`, `term/qt_term_view.cc`, `term/qt_term_view.hh`, `loader/loader_main.cc`, `CMakeLists.txt`, `cmake/install_dependencies.cmake`, `ci/Dockerfile.template`, `main/data/manager.cc`, `main/data/settings.cc/hh`, `main/business/sales.cc/hh`, `main/business/employee.cc/hh`, `main/business/inventory.cc/hh`, `main/business/labor.cc/hh`, `main/data/exception.cc/hh`, `main/data/system.cc/hh`, `main/web/web_server.cc/hh`, `main/hardware/printer.cc/hh`, `zone/payment_zone.cc`.

### Fixed
- **Qt6 Terminal: Multi-Display Forwarding to Remote X11 Server** (2026-06-22)
  - Second terminals launched for a remote X11 server (XSDL, SSH X11 forwarding, or a bare Xorg instance on a different machine) were opening on the local display instead of the target.
  - **Root cause (`term/qt_term_main.cc`):** `!getenv("DISPLAY")` guard prevented `argv[3]` (the target display string passed by `vt_main`) from overriding the inherited `DISPLAY` env var. Since the parent process always has `DISPLAY` set, the remote display argument was silently ignored.
  - **Fix:** Removed the `!getenv("DISPLAY")` condition — `DISPLAY` is now always overridden with `argv[3]` when provided. Also sets `QT_QPA_PLATFORM=xcb` (unless already set) so that Qt6 does not choose the Wayland backend and ignore `DISPLAY` on Wayland-enabled desktops.
  - Files modified: `term/qt_term_main.cc`.

- **Qt6 Terminal: Visual Bugs, Tap-to-Print, and Draggable Toolbar** (2026-06-22)
  - Four persistent visual and interaction bugs were present after the initial Qt6 migration. Root causes traced and fixed with an architectural overhaul of the overlay-window system and page-size handling.
  - **Bug 1 — Random black UI elements:** `target_window_` state was not reset on page transition (`cmdBlankPage`). If a `TERM_SHOWWINDOW` command was lost or delayed (reconnect, partial flush), all subsequent draw commands routed into a window pixmap instead of the main back-buffer, leaving zones black. Fix: `cmdBlankPage` now resets `target_window_ = 0` and hides all overlay windows.
  - **Bug 2 — Tap to print not firing:** The `SrvTouch` protocol message (required by `Terminal::Touch()` for print-button zones) was never sent for mouse clicks. The old code only sent `SrvMouse`, which goes through `Terminal::MouseInput()` and misses the `Touch()` dispatch. Fix: a `moves_count` heuristic distinguishes taps (`moves_count <= 1` before press → `sendTouch()`) from drags (`sendMouse()`). Native touchscreen events always call `sendTouch()`.
  - **Bug 3 — Mouse/touch coordinate misalignment:** The legacy `pageSizeWidth`/`pageSizeHeight` lookup table (18 page sizes) introduced offsets when the selected size didn't exactly match the physical screen. `page_x_`/`page_y_` centering offsets compounded the error. Fix: page size hard-coded to 1920×1080 (see below); `page_x_ = page_y_ = 0` always; single letterbox path eliminates all offset arithmetic.
  - **Bug 4 — Edit toolbar not draggable:** The `FrameMove` flag byte was read from the protocol but discarded (`(void)fr`). The manual drag state machine in `TermWidget` was never triggered because overlay windows intercepted clicks before the main widget. Fix: `WindowWidget` implements its own `mousePressEvent`/`mouseMoveEvent` and calls `move()` directly.
  - **Architectural fix — WindowWidget:** Replaced `WindowLayer` struct (manual pixmap compositing + manual hit-testing in `TermWidget`) with `WindowWidget : QWidget` as native child widgets of `TermWidget`. Qt handles compositing order, hit-testing, coordinate transforms, and mouse routing automatically. Each overlay window manages its own drag state; `QPushButton` children inside `WindowWidget` receive click events natively without any manual hit-rect math.
  - **Page size hard-coded to 1920×1080:** Removed the 18-entry `kPageSizes` lookup table and `pageSizeWidth()`/`pageSizeHeight()`/`pageSizeEnum()` functions. `sendTermInfo()` always reports page size enum 14 (`PAGE_SIZE_1920x1080`) and dimensions 1920×1080 to `vt_main`, ensuring the server always loads 1920×1080 layouts from `vt_data`. Physical screens of any resolution are handled by letterbox/pillarbox scaling in `paintEvent()` (black bars on non-16:9 screens).
  - Files modified: `term/qt_term_view.cc`, `term/qt_term_view.hh`.
